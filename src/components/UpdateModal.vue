<template>
  <ModalShell :show="show" dim="bg-black/60" z="z-[100]" panel-class="p-6 w-[420px] shadow-2xl">
      <div class="flex items-center gap-2 mb-3">
        <span class="text-2xl">🎉</span>
        <h3 class="text-lg font-semibold text-ink">Update Available</h3>
      </div>

      <p class="text-sm text-ink mb-2">
        <strong>TermDrop v{{ version }}</strong> is now available.
      </p>

      <div v-if="notes" class="bg-canvas rounded p-3 mb-4 max-h-40 overflow-y-auto text-xs text-ink-2 leading-relaxed">
        <pre class="whitespace-pre-wrap font-mono">{{ notes }}</pre>
      </div>

      <div v-if="downloading" class="mb-4">
        <div class="flex justify-between text-xs text-ink-2 mb-1">
          <span>Downloading...</span>
          <span>{{ progress }}%</span>
        </div>
        <div class="w-full bg-input rounded-full h-2">
          <div class="bg-accent-solid h-2 rounded-full transition-all duration-200" :style="{ width: progress + '%' }"></div>
        </div>
      </div>

      <div class="flex justify-end gap-2">
        <button
          @click="$emit('close')"
          class="px-4 py-2 text-sm text-ink-2 hover:text-ink rounded"
          :disabled="downloading"
        >Later</button>
        <button
          @click="install"
          class="px-4 py-2 text-sm bg-accent-solid hover:bg-accent-solid-hover text-white rounded disabled:opacity-50"
          :disabled="downloading"
        >{{ downloading ? 'Installing...' : 'Download & Install' }}</button>
      </div>
  </ModalShell>
</template>

<script setup>
import { ref } from 'vue'
import { toast } from '../utils/toast.js'
import ModalShell from './ModalShell.vue'

const props = defineProps({
  show: Boolean,
  version: String,
  notes: String,
  downloadAndInstall: Function,
})

const emit = defineEmits(['close', 'installed'])

const downloading = ref(false)
const progress = ref(0)

async function install() {
  if (!props.downloadAndInstall) return
  downloading.value = true
  progress.value = 0
  try {
    await props.downloadAndInstall((p) => { progress.value = p })
    emit('installed')
  } catch (err) {
    toast('Update failed: ' + err, 'error')
    downloading.value = false
  }
}
</script>
